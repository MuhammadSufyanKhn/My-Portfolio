...
function CertCard({ cert, index, onPreview }: { cert: typeof certifications[0]; index: number; onPreview: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ height: "180px", background: "#f8fafc", border: `1px solid ${cert.color}35`, margin: "16px", borderRadius: "12px", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", transition: "transform 0.4s ease", transform: hovered ? "scale(1.03)" : "scale(1)" }}>
        <object data={cert.file} type="application/pdf" aria-label={`${cert.title} certificate`} style={{ width: "100%", height: "100%", pointerEvents: "none" }}>
          <span style={{ fontSize: "13px", color: cert.color, fontWeight: 700 }}>Open certificate</span>
        </object>
      </div>
...
function CertModal({ cert, onClose }: { cert: typeof certifications[0]; onClose: () => void }) {
  ...
  return createPortal(
    <div style={{ position: "fixed", inset: 0, zIndex: 99990, background: "rgba(0,0,0,0.5)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "24px", opacity: visible ? 1 : 0, transition: "opacity 0.3s ease" }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: "24px", maxWidth: "540px", width: "100%", transform: visible ? "scale(1) translateY(0)" : "scale(0.95) translateY(20px)", transition: "transform 0.4s cubic-bezier(0.23,1,0.32,1)", boxShadow: "0 40px 80px rgba(0,0,0,0.2)", overflow: "hidden" }} onClick={e => e.stopPropagation()}>
        <div style={{ height: "min(55vh, 440px)", background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", borderBottom: `1px solid ${cert.color}20` }}>
          <object data={cert.file} type="application/pdf" aria-label={`${cert.title} certificate`} style={{ width: "100%", height: "100%" }} />
          <button onClick={onClose} style={{ position: "absolute", top: "16px", right: "16px", width: "32px", height: "32px", borderRadius: "50%", border: "none", background: "rgba(0,0,0,0.15)", color: "#555", fontSize: "16px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>×</button>
        </div>
        <div style={{ padding: "28px 32px" }}>
          <h2 style={{ fontFamily: "'Poppins', 'Inter', sans-serif", fontSize: "22px", fontWeight: 800, color: "#1a1a2e", margin: "0 0 8px" }}>{cert.title}</h2>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "16px" }}>
            <span style={{ padding: "4px 12px", borderRadius: "100px", background: `${cert.color}15`, color: cert.color, fontSize: "12px", fontWeight: 600 }}>{cert.organization}</span>
            <span style={{ fontSize: "12px", color: "#475569" }}>Issued {cert.issueDate}</span>
          </div>
          <p style={{ fontSize: "14px", color: "#64748b", lineHeight: 1.6, marginBottom: "20px" }}>{cert.description}</p>
          <div style={{ display: "flex", gap: "10px" }}>
            <a href={cert.file} target="_blank" rel="noopener noreferrer" style={{ flex: 1, textDecoration: "none" }}>
              <button style={{ width: "100%", padding: "11px", borderRadius: "11px", border: "none", background: cert.color, color: "#fff", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>Open Certificate</button>
            </a>
            <button onClick={onClose} style={{ flex: 1, padding: "11px", borderRadius: "11px", border: "1.5px solid rgba(0,0,0,0.1)", background: "transparent", color: "#64748b", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>Close</button>
          </div>
        </div>
      </div>
    </div>,
