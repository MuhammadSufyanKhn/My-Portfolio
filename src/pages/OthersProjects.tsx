      ...
      <div className="page-inner" style={{ maxWidth: "980px", margin: "0 auto", width: "100%", boxSizing: "border-box" }}>
        ...
        <div ref={hero.ref} style={{ marginBottom: "24px", opacity: hero.visible ? 1 : 0, transform: hero.visible ? "translateY(0)" : "translateY(24px)", transition: "all 0.7s ease" }}>
          ...
          <p style={{ fontFamily: "'Newsreader', Georgia, serif", fontSize: "17px", color: "var(--muted)", maxWidth: "560px", lineHeight: 1.7, margin: 0 }}>
            Full-stack web applications, live deployed platforms, and e-commerce systems.
          </p>
        </div>
        ...
        >
          {[
            { label: "Total applications", value: projects.others.length },
            { label: "Live deployed", value: projects.others.filter(p => p.demo).length },
            { label: "Completed", value: projects.others.filter(p => p.status === "Completed").length },
            { label: "Stack", value: "Next.js / React / Full Stack" },
          ].map(stat => (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          {project.demo && (
            <span style={{ padding: "2px 8px", borderRadius: "100px", background: "var(--card)", border: "1px solid var(--line)", color: "#10b981", fontSize: "11px", fontWeight: 700, fontFamily: "'Bricolage Grotesque', sans-serif" }}>
              Live ✓
            </span>
          )}
        ...
        <div style={{ display: "flex", gap: "8px", paddingTop: "14px", borderTop: "1px solid var(--line)" }} className="project-card-footer">
          ...
          {project.demo && (
            <a href={project.demo} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
              <button className="btn-secondary" style={{ height: "38px", padding: "0 12px", fontSize: "12px", justifyContent: "center" }}>
                Live ↗
              </button>
            </a>
          )}
